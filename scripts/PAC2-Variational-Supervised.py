...
def PAC2VI(dataSource = tf.keras.datasets.fashion_mnist, NLabels=10, NPixels=28, algorithm=0, PARTICLES=20, batch_size=100, num_epochs=50, num_hidden_units = 20):
    ...
    py_logprob_2 = tf.expand_dims(py_2.distribution.log_prob(y_batch),1)
    py_logprob_3 = tf.expand_dims(py_3.distribution.log_prob(y_batch),1)
    py_logprob_4 = tf.expand_dims(py_4.distribution.log_prob(y_batch),1)

    logmax = tf.stop_gradient(tf.math.maximum(pylogprob,py_logprob_2)+0.000001)
